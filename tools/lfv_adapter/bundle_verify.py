...
def _validate_descriptor_ref(
    descriptor: Any, path: str
) -> dict[str, str]:
    ...
    if (
        ...
        raise ValidationError(
            f"{path}.serialized_size: expected a non-negative integer"
        )
...
def verify_bundle(
    ...
    *,
    ...
    if bundle["schema_version"] != BUNDLE_SCHEMA:
        raise ValidationError(
            f"bundle.schema_version: expected {BUNDLE_SCHEMA!r}"
        )
    if bundle["canonical_format"] != CANONICAL_FORMAT:
        raise ValidationError(
            f"bundle.canonical_format: expected {CANONICAL_FORMAT!r}"
        )
    experiment = bundle["experiment"]
    if not isinstance(experiment, dict):
        raise ValidationError("bundle.experiment: expected an object")
    algorithm = experiment.get("hash_algorithm")
    digest_ref = validate_artifact_ref(
        bundle["bundle_digest"], "bundle.bundle_digest"
    )
    ...
    if expected_digest != digest_ref:
        raise ValidationError(
            "bundle.bundle_digest: canonical digest mismatch"
        )

    artifacts = bundle["artifacts"]
    if not isinstance(artifacts, dict):
        raise ValidationError("bundle.artifacts: expected an object")
    code_ref = _validate_descriptor_ref(
        artifacts.get("code"), "bundle.artifacts.code"
    )
    ...
    result_ref = _validate_descriptor_ref(
        artifacts.get("result"), "bundle.artifacts.result"
    )
    ...
    if not isinstance(dataset_entries, list) or not dataset_entries:
        raise ValidationError(
            "bundle.artifacts.datasets: expected a non-empty array"
        )
    ...
    for index, item in enumerate(dataset_entries):
        if not isinstance(item, dict) or not isinstance(item.get("id"), str):
            raise ValidationError(
                f"bundle.artifacts.datasets[{index}]: invalid dataset"
            )
        _validate_descriptor_ref(
            item.get("artifact"),
            f"bundle.artifacts.datasets[{index}].artifact",
        )
    ...
    for index, item in enumerate(feature_entries):
        if not isinstance(item, dict) or not isinstance(item.get("name"), str):
            raise ValidationError(
                f"bundle.artifacts.features[{index}]: invalid feature"
            )
        _validate_descriptor_ref(
            item.get("artifact"),
            f"bundle.artifacts.features[{index}].artifact",
        )
    ...
    if (
        ...
        raise ValidationError(
            "bundle.decision.decision_time: invalid timestamp"
        )
    if decision.get("parameter_hash") != parameter_ref["digest"]:
        raise ValidationError(
            "bundle.decision.parameter_hash does not match parameters"
        )
    for dataset in decision.get("datasets", []):
        dataset_id = dataset.get("id")
        if dataset_id not in datasets:
            raise ValidationError(
                f"decision references unknown dataset {dataset_id!r}"
            )
        source = datasets[dataset_id]
        if (
            dataset.get("content_hash")
            != source["artifact"]["ref"]["digest"]
        ):
            raise ValidationError(
                f"decision dataset {dataset_id!r}: hash mismatch"
            )
        if dataset.get("observed_at") != source.get("observed_at"):
            raise ValidationError(
                f"decision dataset {dataset_id!r}: observed_at mismatch"
            )
        if dataset.get("available_at") != source.get("available_at"):
            raise ValidationError(
                f"decision dataset {dataset_id!r}: available_at mismatch"
            )
        if dataset.get("available_at", decision_time + 1) > decision_time:
            raise ValidationError(
                f"decision dataset {dataset_id!r}: future information"
            )
    if decision.get("dataset_ids") != [
        item.get("id") for item in decision.get("datasets", [])
    ]:
        raise ValidationError(
            "decision.dataset_ids does not match decision.datasets"
        )
    ...
    for name, feature in features.items():
        ...
        if (
            ...
            raise ValidationError(
                f"feature {name!r}: invalid generated_at"
            )
        ...
        for reference in feature.get("inputs", []):
            ...
            if kind == "dataset":
                if (
                    target not in datasets
                    or datasets[target]["available_at"] > generated
                ):
                    raise ValidationError(
                        f"feature {name!r}: temporally invalid dataset input"
                    )
                expected_input_hashes.append(
                    datasets[target]["artifact"]["ref"]["digest"]
                )
            elif kind == "feature":
                if target not in feature_times:
                    # Artifacts are required to be emitted in topological order.
                    raise ValidationError(
                        f"feature {name!r}: non-topological dependency"
                    )
                if feature_times[target] > generated:
                    raise ValidationError(
                        f"feature {name!r}: later feature dependency"
                    )
                expected_input_hashes.append(feature_hashes[target])
            else:
                raise ValidationError(
                    f"feature {name!r}: invalid lineage reference"
                )
        if feature.get("input_hashes") != expected_input_hashes:
            raise ValidationError(
                f"feature {name!r}: lineage hash mismatch"
            )
    ...
    for feature in decision.get("features", []):
        ...
        if name not in features:
            raise ValidationError(
                f"decision references unknown feature {name!r}"
            )
        source = features[name]
        if (
            feature.get("output_hash")
            != source["artifact"]["ref"]["digest"]
        ):
            raise ValidationError(
                f"decision feature {name!r}: output hash mismatch"
            )
        if feature.get("input_hashes") != source.get("input_hashes"):
            raise ValidationError(
                f"decision feature {name!r}: input hash mismatch"
            )
        if feature.get("code_hash") != source.get("code_hash"):
            raise ValidationError(
                f"decision feature {name!r}: code hash mismatch"
            )
        if feature.get("generated_at") != source.get("generated_at"):
            raise ValidationError(
                f"decision feature {name!r}: generated_at mismatch"
            )
        if feature.get("generated_at", decision_time + 1) > decision_time:
            raise ValidationError(
                f"decision feature {name!r}: future information"
            )
    if decision.get("feature_names") != [
        item.get("name") for item in decision.get("features", [])
    ]:
        raise ValidationError(
            "decision.feature_names does not match decision.features"
        )
    ...
    if isinstance(selected_index, bool) or not isinstance(
        ...
        raise ValidationError(
            "bundle.selected_trial_index: expected an integer"
        )
    if selected_index < 0 or selected_index >= len(ledger["entries"]):
        raise ValidationError(
            "bundle.selected_trial_index: out of range"
        )
    ...
    if selected["registered_at"] > decision_time:
        raise ValidationError(
            "selected trial was registered after the cutoff"
        )
    claim = bundle["claim"]
    ...
    if claim.get("result_hash") != result_ref["digest"]:
        raise ValidationError(
            "claim result hash does not match the result artifact"
        )
