...
from tools.selective_receipt.errors import (
    ValidationError as SelectiveReceiptValidationError,
)
from tools.selective_receipt.merkle import (
    build_tree,
    proof as merkle_proof,
    verify as verify_merkle,
)
...
def _scalar(seed: bytes, label: bytes, q: int) -> int:
    value = int.from_bytes(
        hmac.new(seed, label, hashlib.sha256).digest(), "big"
    ) % q
    return value or 1
...
def _leaf(action: str, committed: int) -> bytes:
    return hashlib.sha256(
        b"\x00"
        + canonical_bytes(
            {"action": action, "commitment": format(committed, "x")}
        )
    ).digest()
...
def _sign(
    payload: dict[str, Any], private_key: Path, openssl: str
) -> str:
    with tempfile.TemporaryDirectory(prefix="lfv-zk-sign-") as temporary:
        ...
        if signature_result.returncode != 0:
            raise ValidationError(
                signature_result.stderr.decode("utf-8", errors="replace")
            )
...
def _build_merkle_tree(leaves: list[bytes]) -> list[list[bytes]]:
    ...
    except SelectiveReceiptValidationError as exc:
...
def _merkle_proof(
    ...
    try:
        return merkle_proof(levels, index)
    except SelectiveReceiptValidationError as exc:
        raise ValidationError(str(exc)) from exc


def _verify_merkle_membership(
    leaf: bytes, path: list[dict[str, str]], expected_root: str
) -> None:
    try:
        verify_merkle(leaf, path, expected_root)
    except SelectiveReceiptValidationError as exc:
        raise ValidationError(str(exc)) from exc
...
def issue_receipt(
    ...
    *,
    ...
    if unknown:
        raise ValidationError(
            f"execution log contains unknown actions: {sorted(unknown)}"
        )
    ...
    blindings = {
        action: _scalar(
            blinding_seed,
            b"LFV\x00BLIND\x00" + action.encode(),
            parameters.q,
        )
    ...
    levels = _build_merkle_tree(
        [_leaf(action, commitments[action]) for action in policy.action_universe]
    )
    root = levels[-1][0].hex()
    disclosures = []
    for action in policy.forbidden_actions:
        ...
        action_seed = (
            ...
            else hmac.new(nonce_seed, action.encode(), hashlib.sha256).digest()
        )
        disclosures.append(
            {
                "action": action,
                "commitment": format(commitments[action], "x"),
                "leaf_index": index,
                "audit_path": _merkle_proof(levels, index),
    ...
    receipt["signature_base64"] = _sign(
        _signed_payload(receipt), private_key, openssl
    )
    return receipt
...
def verify_receipt(
    ...
    *,
    ...
    if (
        receipt["runner_id"] != policy.runner_id
        or receipt["trust_domain"] != policy.trust_domain
    ):
    ...
    if receipt["public_key_sha256"] != _public_key_hash(public_key):
        raise ValidationError(
            "verifier-selected public key does not match receipt"
        )
    proofs = receipt["proofs"]
    if not isinstance(proofs, list):
        raise ValidationError("receipt proofs must be an array")
    actions = [
        item.get("action") for item in proofs if isinstance(item, dict)
    ]
    if actions != list(policy.forbidden_actions):
        raise ValidationError(
            "receipt must prove every forbidden class and no others"
        )
    for item in proofs:
        if set(item) != {
            "action",
            ...
            raise ValidationError(
                "private proof disclosure has invalid fields"
            )
        action = item["action"]
        try:
        ...
        if item["leaf_index"] != index:
            raise ValidationError(
                "commitment index does not match action universe"
            )
        _verify_merkle_membership(
            _leaf(action, committed),
            item["audit_path"],
            receipt["root_sha256"],
        )
        context = {
