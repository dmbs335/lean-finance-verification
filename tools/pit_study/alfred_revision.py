...
CONFIG_SCHEMA = "lfv-alfred-revision-study-v1"
PACKAGE_SCHEMA = "lfv-alfred-vintage-package-v1"
REPORT_SCHEMA = "lfv-alfred-revision-leakage-report-v1"
...
class VintagePackage:
    ...
    def by_key(self) -> dict[tuple[str, str], SnapshotRef]:
        return {
            (item.kind, item.decision_date): item
...
def load_config(path: Path) -> StudyConfig:
    ...
    for index, item in enumerate(decisions_raw):
        obj = _object(item, f"$.decisions[{index}]")
        if set(obj) != {"decision_date", "realized_return_bps"}:
            raise ValidationError(
                f"$.decisions[{index}]: fields do not match decision schema"
            )
        decisions.append(
            DecisionSpec(
                decision_date=_iso_date(
                    obj["decision_date"],
                    f"$.decisions[{index}].decision_date",
                ),
                realized_return_bps=_integer(
                    obj["realized_return_bps"],
                    f"$.decisions[{index}].realized_return_bps",
                ),
            )
        )
    decision_dates = [item.decision_date for item in decisions]
    if decision_dates != sorted(decision_dates):
        raise ValidationError("$.decisions: dates must be sorted")
    if len(set(decision_dates)) != len(decision_dates):
        raise ValidationError("$.decisions: dates must be unique")
    ...
    if any(
        latest_vintage_date <= decision_date
        for decision_date in decision_dates
    ):
        raise ValidationError(
            "$.latest_vintage_date must be later than every decision date"
        )
    return StudyConfig(
        source=path.resolve(),
...
def _safe_package_path(root: Path, relative: str, path: str) -> Path:
    ...
    if not candidate.is_file():
        raise ValidationError(f"{path}: missing response file {relative}")
    return candidate
...
def load_package(path: Path, config: StudyConfig) -> VintagePackage:
    ...
    for index, item in enumerate(responses_raw):
        ...
        decision_date = _iso_date(
            obj["decision_date"], f"{item_path}.decision_date"
        )
        vintage_date = _iso_date(
            obj["vintage_date"], f"{item_path}.vintage_date"
        )
        if kind == "realtime" and vintage_date != decision_date:
            raise ValidationError(
                f"{item_path}: realtime vintage must equal decision date"
            )
    ...
    keys = [(item.kind, item.decision_date) for item in responses]
    if len(set(keys)) != len(keys):
        raise ValidationError("$.responses: duplicate kind/decision pair")
    expected_keys = {
        (kind, decision.decision_date)
    ...
    if set(keys) != expected_keys:
        ...
        raise ValidationError(
            f"$.responses: incomplete decision coverage; missing={missing} extra={extra}"
        )
...
def _load_observations(
    ...
    if not isinstance(observations_raw, list):
        raise ValidationError(f"{ref.relative_path}: observations must be an array")
    ...
    for index, item in enumerate(observations_raw):
        ...
        if observation_date in seen_dates:
            raise ValidationError(
                f"{ref.relative_path}: duplicate observation date {observation_date}"
            )
        seen_dates.add(observation_date)
        if observation_date < ref.decision_date:
            observations.append(
                Observation(
                    observation_date=observation_date,
                    value_scaled=_scale_value(
                        value_text, config.value_scale, f"{item_path}.value"
                    ),
                )
            )
    ...
    if len(observations) < 2:
        raise ValidationError(
            f"{ref.relative_path}: fewer than two observations precede decision"
        )
    return observations
...
def run_study(
    ...
    realtime_previous: int | None = None
    revision_previous: int | None = None
    naive_previous: int | None = None

    for spec in config.decisions:
        realtime_ref = by_key[("realtime", spec.decision_date)]
        latest_ref = by_key[("latest", spec.decision_date)]
        realtime_observations = _load_observations(realtime_ref, config)
        latest_observations = _load_observations(latest_ref, config)
        realtime_pair = tuple(realtime_observations[-2:])
        latest_by_date = {
            observation.observation_date: observation
            for observation in latest_observations
        }
        try:
            latest_same_pair = tuple(
                latest_by_date[observation.observation_date]
                for observation in realtime_pair
            )
        ...
        realtime = _summary_from_pair(
            realtime_ref, realtime_pair, config  # type: ignore[arg-type]
        )
        latest_same = _summary_from_pair(
            latest_ref, latest_same_pair, config  # type: ignore[arg-type]
        )
        latest_naive = _summary_from_pair(
            latest_ref, latest_naive_pair, config  # type: ignore[arg-type]
        )

        realtime_turnover = _turnover_cost(
            realtime_previous,
        ...
        realtime_return = (
            int(realtime["position"]) * spec.realized_return_bps
            - realtime_turnover
        )
        revision_return = (
            int(latest_same["position"]) * spec.realized_return_bps
            - revision_turnover
        )
        naive_return = (
        ...
        availability_changed = (
            realtime["observation_dates"]
            != latest_naive["observation_dates"]
        )
        selected_values_revised = (
            realtime["values_scaled"] != latest_same["values_scaled"]
        )
        revision_signal_changed = (
            realtime["signal_scaled"] != latest_same["signal_scaled"]
        )
        revision_position_changed = (
            realtime["position"] != latest_same["position"]
        )
        naive_position_changed = (
            realtime["position"] != latest_naive["position"]
        )
        decisions.append(
            {
                ...
                "realtime": realtime,
                "latest_same_observations": latest_same,
                "latest_naive": latest_naive,
                "realtime_vintage_available_at_decision": (
                    realtime_ref.vintage_date <= spec.decision_date
                ),
                "latest_vintage_is_post_decision": (
                    spec.decision_date < latest_ref.vintage_date
                ),
                "availability_changed": availability_changed,
                "selected_values_revised": selected_values_revised,
                "revision_signal_changed": revision_signal_changed,
                "revision_position_changed": revision_position_changed,
                "naive_position_changed": naive_position_changed,
                "realtime_turnover_cost_bps": realtime_turnover,
                "revision_only_turnover_cost_bps": revision_turnover,
                "latest_naive_turnover_cost_bps": naive_turnover,
                "realtime_strategy_return_bps": realtime_return,
                "revision_only_strategy_return_bps": revision_return,
                "latest_naive_strategy_return_bps": naive_return,
                "revision_only_leakage_bps": (
                    revision_return - realtime_return
                ),
                "revision_plus_availability_leakage_bps": (
                    naive_return - realtime_return
                ),
            }
        )
        realtime_total += realtime_return
        revision_only_total += revision_return
        latest_naive_total += naive_return
        realtime_previous = int(realtime["position"])
        revision_previous = int(latest_same["position"])
    ...
    report: dict[str, Any] = {
        ...
        "api_contract": {
            ...
            "output_type": 1,
            "realtime_selection": (
                "query realtime_start=realtime_end at each decision date"
            ),
        },
        "assurance": {
            "realtime_path": "strictPointInTimeAsOfDecision",
            "revision_only_counterfactual": (
                "postDecisionLatestValuesOnDecisionTimeObservations"
            ),
            "latest_naive_counterfactual": (
                "postDecisionLatestValuesAndObservationAvailability"
            ),
            "latest_counterfactuals_are_strict_point_in_time": False,
        },
        ...
        "decisions": decisions,
        "aggregate": {
            "realtime_total_return_bps": realtime_total,
            "revision_only_total_return_bps": revision_only_total,
            "latest_naive_total_return_bps": latest_naive_total,
            "revision_only_leakage_bps": (
                revision_only_total - realtime_total
            ),
            "revision_plus_availability_leakage_bps": (
                latest_naive_total - realtime_total
            ),
            ...
            "selected_value_revision_count": sum(
                1 for item in decisions if item["selected_values_revised"]
            ),
            "all_realtime_vintages_available": all(
                item["realtime_vintage_available_at_decision"]
                for item in decisions
            ),
            "all_latest_vintages_post_decision": all(
                item["latest_vintage_is_post_decision"]
                for item in decisions
            ),
        },
        "source_responses": [
            ...
            for item in sorted(
                package.responses,
                key=lambda item: (item.decision_date, item.kind),
            )
        ],
        "interpretation": (
            "comparison of a decision-time ALFRED vintage path, a pure "
            "revision counterfactual on the same observation dates, and a "
            "naive latest-vintage reconstruction that may also leak release "
            "availability; not a future-profitability claim"
        ),
    }
...
def download_package(
    ...
    maximum_decision = config.decisions[-1].decision_date
    common = {
        "series_id": config.series_id,
        "file_type": "json",
        "output_type": "1",
    ...
    latest_raw = _download_json(
        {
            **common,
            ...
            "observation_end": maximum_decision,
        },
        api_key,
    )
    ...
    for decision in config.decisions:
        realtime_relative = f"responses/realtime-{decision.decision_date}.json"
        realtime_path = out_dir / realtime_relative
        realtime_raw = _download_json(
            {
                **common,
                "realtime_start": decision.decision_date,
                "realtime_end": decision.decision_date,
                "observation_end": decision.decision_date,
            },
            api_key,
        )
        ...
        refs.append(
            {
                ...
                "vintage_date": decision.decision_date,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "download":
            ...
            manifest = download_package(config, args.out_dir, key)
            ...
            if args.command == "analyze":
                ...
                print(
                    f"realtime={aggregate['realtime_total_return_bps']}bps "
                    f"revision_only="
                    f"{aggregate['revision_only_total_return_bps']}bps "
                    f"naive_latest="
                    f"{aggregate['latest_naive_total_return_bps']}bps "
                    f"revision_leakage="
                    f"{aggregate['revision_only_leakage_bps']}bps"
                )
            else:
