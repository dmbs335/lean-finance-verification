...
from .errors import EvidencePortfolioError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-evidence-portfolio")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(
                f"raw={report['raw_optimum']['strategies']} "
                f"adjusted={report['evidence_adjusted_optimum']['strategies']} "
                f"gain={report['adjusted_optimum_score_gain']}"
            )
        else:
            verify(problem, load_json(args.report))
            print(f"verified {args.report}")
        return 0
    except (EvidencePortfolioError, OSError) as exc:
