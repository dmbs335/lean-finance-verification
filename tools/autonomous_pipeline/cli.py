...
from .errors import AutonomousPipelineError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-autonomous-pipeline")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(f"selected={selected['channels']} cost={selected['cost']}")
        else:
            verify(problem, load_json(args.report))
            print(f"verified {args.report}")
        return 0
    except (AutonomousPipelineError, CanonicalValidationError, OSError) as exc:
