...
from .errors import AnytimePolicyMonitorError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-anytime-policy-monitor")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            evidence = report["anytime_evidence"]
            print(
                f"max_e={evidence['maximum_e_value']['numerator']}/"
                f"{evidence['maximum_e_value']['denominator']} "
                f"crossing={evidence['first_crossing_observation']} "
        ...
        return 0
    except (AnytimePolicyMonitorError, CanonicalValidationError, OSError) as exc:
