...
from .errors import CertifiableAlphaIntervalError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-certifiable-alpha-interval")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(
                ...
                f"interval={selected['interval_bps']}"
            )
        else:
            verify(problem, load_json(args.report))
            print(f"verified {args.report}")
        return 0
    except (CertifiableAlphaIntervalError, OSError) as exc:
