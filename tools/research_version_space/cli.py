...
from .errors import ResearchVersionSpaceError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-research-version-space")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(
                ...
                f"no_evidence={report['no_evidence']['range']} "
                f"selected={selected['channels']} "
                f"range={selected['range']} cost={selected['cost']}"
            )
        else:
