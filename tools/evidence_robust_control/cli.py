...
from .errors import EvidenceRobustControlError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-evidence-robust-control")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(
                f"decision={report['decision']['kind']} "
                f"value={report['decision']['selected_net_value_bps']}"
            )
        else:
            verify(problem, load_json(args.report))
            print(f"verified {args.report}")
        return 0
    except (EvidenceRobustControlError, CanonicalValidationError, OSError) as exc:
