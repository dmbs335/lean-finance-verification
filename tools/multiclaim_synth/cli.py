...
from .errors import MultiClaimError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-multiclaim-synth")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(
                f"union={report['claim_specific_union']['cost']} "
                f"global={report['global']['selected']['cost']} "
                f"savings={report['synergy_savings']}"
            )
        else:
            verify(problem, load_json(args.report))
            print(f"verified {args.report}")
        return 0
    except (MultiClaimError, OSError) as exc:
