...
def _formula(value: Any) -> FormulaDefinition:
    ...
    return FormulaDefinition(
        ...
        registered_at=_natural(
            obj["registered_at"], "$.formula.registered_at"
        ),
...
def load_problem(path: Path) -> Problem:
    ...
    for index, item in enumerate(applications_raw):
        ...
        expected = {
            ...
            "output_artifact_sha256", "output_generated_at",
        }
        if set(obj) != expected:
        ...
        applications.append(
            Application(
                ...
                output_artifact_sha256=_hash(
                    obj["output_artifact_sha256"],
                    f"{item_path}.output_artifact_sha256",
                ),
                output_generated_at=_natural(
                    obj["output_generated_at"],
                    f"{item_path}.output_generated_at",
                ),
            )
        )
