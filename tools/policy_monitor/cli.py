...
from .errors import PolicyMonitorError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-policy-monitor")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            monitor = report["off_policy"]
            print(
                f"lcb={monitor['improvement_interval_bps'][0]} "
                f"ess={monitor['effective_sample_size']['numerator']}/"
                f"{monitor['effective_sample_size']['denominator']} "
        ...
        return 0
    except (PolicyMonitorError, CanonicalValidationError, OSError) as exc:
