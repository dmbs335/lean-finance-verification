...
from .errors import BeliefControlError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-belief-control")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(
                f"prior={report['prior']['control']['selected']['action']} "
                f"query={report['query']['decision']} "
                f"voi={report['query']['robust_value_of_information_bps']}"
            )
        else:
            verify(problem, load_json(args.report))
            print(f"verified {args.report}")
        return 0
    except (BeliefControlError, CanonicalValidationError, OSError) as exc:
