...
from .errors import ExecutionHarnessError
from .model import load_problem
from .solver import solve, verify


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="lfv-proof-carrying-execution")
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(
                f"filled={recon['filled_qty']} "
                f"cash_delta={recon['cash_delta']} "
                f"inventory_delta={recon['inventory_delta']}"
            )
        else:
            verify(problem, load_json(args.report))
            print(f"verified {args.report}")
        return 0
    except (ExecutionHarnessError, CanonicalValidationError, OSError) as exc:
