...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="lfv-robust-evidence-synth",
        description=(
            "Exact minimum-cost evidence synthesis under bounded "
    ...
    synth = subparsers.add_parser(
        "synth", help="solve a robust evidence model and emit JSON plus Lean"
    )
    ...
    verify = subparsers.add_parser(
        "verify", help="recompute and verify a robust synthesis certificate"
    )
    ...
    check = subparsers.add_parser(
        "check-generated",
        help="fail when checked-in robust JSON or Lean has drifted",
    )
...
def _command_check(args: argparse.Namespace) -> None:
    ...
    if args.certificate.read_bytes() != canonical_bytes(certificate):
        raise SynthesisError(
            f"robust synthesis certificate drift: regenerate {args.certificate}"
        )
    if args.lean.read_text(encoding="utf-8") != lean_source:
        raise SynthesisError(
            f"generated robust Lean drift: regenerate {args.lean}"
        )
    print("generated robust evidence artifacts are reproducible")
