...
REPORT_SCHEMA = "lfv-proof-carrying-research-agent-report-v3"
STAGES = [
...
def run(plan: Plan) -> dict[str, Any]:
    ...
    alpha_interval = solve_alpha_interval(load_alpha_interval(plan.analyses.certifiable_alpha_interval))
    portfolio = solve_portfolio(load_portfolio(plan.analyses.evidence_portfolio))
    crowding = simulate_crowding(load_crowding(plan.analyses.certifiability_crowding))
    liquidation = simulate_liquidation(load_liquidation(plan.analyses.epistemic_liquidation))
    event_study = analyze_event_study(load_event_study(plan.analyses.epistemic_event_study))

    selected_alpha = fake_alpha["synthesis"]["selected"]
    exact_recovery = bool(selected_alpha["verifies"]) and all(
        evaluation["exact_recovery"] for evaluation in selected_alpha["evaluations"]
    )
    ...
    alpha_interval_gate = (
        ...
        and (positive_lower or not plan.gates.require_positive_certifiable_lower_bound)
    )

    portfolio_gain = portfolio["adjusted_optimum_score_gain"]
    portfolio_gate = portfolio_gain >= plan.gates.minimum_adjusted_portfolio_gain
    ...
    liquidation_gate = hidden_pairs >= plan.gates.minimum_hidden_common_risk_pairs
    ...
    gates = {
        ...
        "alpha_interval": {
            ...
            "maximum_width_bps": plan.gates.maximum_certifiable_interval_width_bps,
            "positive_lower_bound": positive_lower,
            "positive_lower_bound_required": plan.gates.require_positive_certifiable_lower_bound,
        },
        "portfolio": {
            "passed": portfolio_gate,
            "adjusted_score_gain": portfolio_gain,
            "raw_selection": portfolio["raw_optimum"]["strategies"],
            "adjusted_selection": portfolio["evidence_adjusted_optimum"]["strategies"],
        },
        "crowding": {
        ...
        "liquidation": {
            ...
            "final_market_impact_bps": liquidation["final_market_impact_bps"],
        },
        "event_study": {
            "passed": event_study_gate,
            "accepted_controlled": event_accepted,
            "average_event_did_bps": event_average,
            "minimum_average_event_did_bps": plan.gates.minimum_event_study_average_did_bps,
...
def verify(plan: Plan, report: Any) -> dict[str, Any]:
    expected = run(plan)
    if report != expected:
        raise ValidationError("research-agent report does not match exact recomputation")
    return expected
