...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(
                f"safe={aggregate['causal_and_pure_count']}/"
                f"{aggregate['pipeline_count']} "
                f"unsafe={aggregate['unsafe_pipeline_count']}"
            )
        else:
            verify(problem, load_json(args.report))
            print(f"verified {args.report}")
        return 0
    except (TemporalNoninterferenceError, CanonicalValidationError, OSError) as exc:
